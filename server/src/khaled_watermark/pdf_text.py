    ...
    for page_no, page in enumerate(doc):
        ...
        for stream_no, xref in enumerate(page.get_contents()):
            if xref in seen_xrefs:
                raise UnsupportedText("shared page content streams are unsupported")
            seen_xrefs.add(xref)
            data = doc.xref_stream(xref)
            tokens = tokenize(data)
            # Content streams containing transformed drawing need a graphics
            # state interpreter; the first version only accepts translations.
            transformed = any(
                token.kind == "word" and token.value == b"cm" and (
                    i < 6 or any(_num(tokens, i - 6 + j) is None or
                                  abs(float(_num(tokens, i - 6 + j)) - expected) > 1e-6
            ...
            for i, token in enumerate(tokens):
                ...
                if word == b"BT":
                    ...
                    inside = False
                elif word == b"Tf" and inside and i >= 2:
